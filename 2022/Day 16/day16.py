...
def makeMove(minsLeft, pos, graph, flows, states, prevRoom="None"):
    ...
    for adjacent in graph[pos]:
        if adjacent != prevRoom: # Don't go back to the exact same place we came from
            possibleFlows.append(makeMove(minsLeft-1, adjacent, graph, flows, states, prevRoom=pos))

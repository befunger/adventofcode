...
public:
    vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
        /*Same as 102 but every other level is reverse order*/
        ...
        bool reverse_level = false; // Toggle each level to reverse/unreverse
        ...
        while(!nodes.empty()){
            ...
            if(level_size == 0){ // When finished with one level, start next level and reset size
                ...
                reverse_level = !reverse_level; // Swap each layer if we reverse or not
